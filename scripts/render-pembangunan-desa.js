// Module: render-pembangunan-desa.js
// Fetches pembangunan-desa.json and renders hierarchical table

const PEMBANGUNAN_URL = '/assets/data/pembangunan-desa.json';

async function fetchPembangunanData() {
  try {
    const res = await fetch(PEMBANGUNAN_URL, { cache: 'no-store' });
    if (!res.ok) throw new Error('Failed to fetch pembangunan desa data');
    return await res.json();
  } catch (err) {
    console.error('render-pembangunan-desa:', err);
    return null;
  }
}

async function renderPembangunanTable() {
  const container = document.getElementById('pembangunan-table-container');
  if (!container) return;

  const data = await fetchPembangunanData();
  if (!data || !Array.isArray(data.data) || data.data.length === 0) {
    container.innerHTML = '<p class="text-muted">Data pembangunan desa tidak tersedia</p>';
    return;
  }
  ...
  const headers = ['No', 'Bidang Pembangunan', 'Keterangan'];
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', renderPembangunanTable);
} else {
