// Module: render-potensi-pemberdayaan-masyarakat.js
// Fetches pemberdayaan-masyarakat.json and renders hierarchical table

const PEMBERDAYAAN_URL = '/assets/data/pemberdayaan-masyarakat.json';

async function fetchPemberdayaanData() {
  try {
    const res = await fetch(PEMBERDAYAAN_URL, { cache: 'no-store' });
    if (!res.ok) throw new Error('Failed to fetch pemberdayaan masyarakat data');
    return await res.json();
  } catch (err) {
    console.error('render-potensi-pemberdayaan-masyarakat:', err);
    return null;
  }
}

async function renderPemberdayaanTable() {
  const container = document.getElementById('pemberdayaan-table-container');
  if (!container) return;

  const data = await fetchPemberdayaanData();
  if (!data || !Array.isArray(data.data) || data.data.length === 0) {
    container.innerHTML = '<p class="text-muted">Data pemberdayaan masyarakat tidak tersedia</p>';
    return;
  }
  ...
  const headers = ['No', 'Uraian Sumber Daya Pemberdayaan', 'Jumlah'];
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', renderPemberdayaanTable);
} else {
  renderPemberdayaanTable();
}
